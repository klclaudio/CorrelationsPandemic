...
mat_correl_csv <- c(); # matriz de correlaçoes rmat_correl_csv <- c(); # matriz de correlaçoes referentes aos dias avaliados após a 5a ocorrência
 ...
 #correlações do sistema ABO com distinção dos fatores Rh
 for( jj in c(1:9) ) {
      mat_correl_csv <- cbind( mat_correl_csv, correlacoes[,jj] );
      mat_correl_csv <- cbind( mat_correl_csv, correlacoest[,jj] );
      mat_ci_csv <- cbind( mat_ci_csv, ( correlacoest_CI [ (N_i*( 2*(jj-1)) + 1) : (N_i*(2*jj -1)),] ) ); 
      mat_ci_csv <- cbind( mat_ci_csv, ( correlacoest_CI [ (N_i*(2*jj-1)+1) : (N_i*( 2*jj )),] )  );
 }

      #correlações do sistema ABO sem distinção dos fatores Rh
