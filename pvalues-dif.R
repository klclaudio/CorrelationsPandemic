...
#Comparative plots 12 days
  ...
  plot( correlacoest_abo_rh[ 1:xlimite, "Rh-"], ylab = "", xlab = "", cex.main = ftsize, cex.lab = ftsizelb, cex.axis = ftsizeax,
   ...
   plot( correlacoest_abo_rh[ , "Rh-"][1:xlimite], ylab = "", xlab = "", cex.main = ftsize, cex.lab = ftsizelb, cex.axis = ftsizeax,
         xlim = c(1, xlimite), ylim = c(-1, 1), cex.main = ftsize, cex.lab = ftsizelb, cex.axis = ftsizeax, col = color_l2[2],
         type = "l", lty = 2, pch = "o"
        )
      par(new = TRUE)
...
i_min_trhp <- which.min( correlacoest_abo_rh[ , "Rh+"][4:25] ) +3
min_trhp <- min( correlacoest_abo_rh[ , "Rh+"][4:25] )

i_min_rhp <- which.min( correlacoest_abo_rh[ , "Rh+"][4:25] ) + 3
min_rhp <- min( correlacoest_abo_rh[ , "Rh+"][4:25] )


i_max_trhn <- which.max( correlacoest_abo_rh[ , "Rh-"][4:50] ) + 3
max_trhn <- max( correlacoest_abo_rh[ , "Rh-"][4:50] )


i_max_trhp <- which.max( correlacoest_abo_rh[ , "Rh+"][4:50] ) + 3
max_trhp <- max( correlacoest_abo_rh[ , "Rh+"][4:50] )


i_max_rhn <- which.max( correlacoest_abo_rh[ , "Rh-"][4:460] ) + 3
max_rhn <- max( correlacoest_abo_rh[ , "Rh-"][4:460] )


i_max_rhp <- which.max(correlacoest_abo_rh[ , "Rh+"][4:460]) + 3
max_rhp <- max(correlacoest_abo_rh[ , "Rh+"][4:460])
