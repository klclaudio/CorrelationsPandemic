...
# V0.3 next steps: PCA analyses. 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
#
...
pval  <- c();  errors_a <- c(); 
wstat <- c(); errors_lin <- c();

covid_ll <- c(); coeff_aux <- c();
coeff_a <- c(); coeff_lin <- c();

r2 <- c(); mat_coeffs_csv <- c();

mat_coefilin_csv  <- c(); mat_coefia_csv <- c();
mat_statslin_csv <- c();  mat_statsa_csv <- c();
mat_stderrlin_csv <- c(); mat_stderra_csv <- c();


# logdata: 0- without log  transformation,  1- data with log transformation
if( logdata == 0 ) {
		...
		for( i in c(1:N_i) ) {
			...
			for( k in c(1:l) ) {
			  ...
			  if( aux[k, 4] == namesdatac[m] && is.na( aux[k, 8] ) == FALSE ) {
				...
				# if( is.na(aux[k, 8]) == FALSE) { # all countries
				...
				if( logdata == 1 ) {
				     ...
				     # rate_covid <- cbind( rate_covid, log(as.matrix(aux[k, 8])/(as.numeric(aux[k, "population"])/10^6)) ); # 
				     # rate_covid <- cbind( rate_covid, log(as.matrix(aux[k, 8])/(as.numeric(aux[k, "population"])/10^6)) ); # daily rates
					...
					# Cálculo com taxa por milhao a normalidadede dos dados ficam comprometidos            
				if (tdpm == 1) {
